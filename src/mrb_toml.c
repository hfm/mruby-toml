...
static mrb_value mrb_toml_hello(mrb_state *mrb, mrb_value self)
{
  mrb_toml_data *data = DATA_PTR(self);

  return mrb_str_new(mrb, data->str, data->len);
}

static mrb_value mrb_toml_hi(mrb_state *mrb, mrb_value self)
{
...
void mrb_mruby_toml_gem_init(mrb_state *mrb)
{
  ...
  mrb_define_method(mrb, toml, "hello", mrb_toml_hello, MRB_ARGS_NONE());
  mrb_define_class_method(mrb, toml, "hi", mrb_toml_hi, MRB_ARGS_NONE());
  DONE;
}
